...
def main():
    ...
    out = get_log_dir('combined-fcn-paf-full-adam-vallog-classes', args.config, cfg)
    resume = args.resume
    ...
    train_loader = torch.utils.data.DataLoader(
        torchfcn.datasets.Combined_test(root, split='train', transform=True,test_par = 1.),
    ...
    model = torchfcn.models.COMB_MULT()
    start_epoch = 0
    start_iteration = 0
    if resume:
    ...
    trainer = torchfcn.TrainerCOMB_mult(
        cuda=cuda,
        model=model,        
        optimizer=optim,
        scheduler=scheduler,
