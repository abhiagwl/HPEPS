...
def main():
    ...
    out = get_log_dir('paf32s_fixloss', args.config, cfg)
    resume = args.resume
    ...
    train_loader = torch.utils.data.DataLoader(
        torchfcn.datasets.PAF_loader_test(root, split='train', transform=True,test_par = 0.1),
        batch_size=1, shuffle=True, **kwargs)
    val_loader = torch.utils.data.DataLoader(
        torchfcn.datasets.PAF_loader_test(
            root, split='val', transform=True,test_par = 1.),
        batch_size=1, shuffle=False, **kwargs)

    # 2. model

    model = torchfcn.models.PAF_MULT()
    start_epoch = 0
    start_iteration = 0
    if resume:
    ...
    trainer = torchfcn.TrainerPAF_mult(
        cuda=cuda,
        model=model,        
        optimizer=optim,
        scheduler=scheduler,
