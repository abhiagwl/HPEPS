...
class FCN32s_test(nn.Module):
    ...
    def __init__(self, n_class=21):
        super(FCN32s_test, self).__init__()
        # conv1
...
class FCN32s_wo_crop(nn.Module):
    ...
    def __init__(self, n_class=21):
        super(FCN32s_wo_crop, self).__init__()
        # conv1
    ...
    def forward(self, x):
        ...
        # n_t,c_t,h_t,w_t = x.size()
        # n,c,h,w = h.size()
        
        # pad_w1 = np.ceil((w-w_t)/2).astype(np.int32)
        # pad_h1 = np.ceil((h-h_t)/2).astype(np.int32)
        # h = h[:, :, 19:19 + x.size()[2], 19:19 + x.size()[3]].contiguous()
        h = h[:, :, (h.size()[2]-x.size()[2])/2:(h.size()[2]-x.size()[2])/2 + x.size()[2], (h.size()[3]-x.size()[3])/2:(h.size()[3]-x.size()[3])/2 + x.size()[3]].contiguous()
...
class PAF32s_test(nn.Module):
    ...
    def __init__(self, n_class=21):
        super(PAF32s_test, self).__init__()
        # conv1
        self.conv1_1 = nn.Conv2d(3, 64, 3, padding=100)
        self.relu1_1 = nn.ReLU(inplace=True)
        self.conv1_2 = nn.Conv2d(64, 64, 3, padding=1)
