...
class TrainerCOMB_mult(object):

    def __init__(self, cuda, model, optimizer,scheduler,
                 train_loader, val_loader, out, max_iter,
                 size_average=False, interval_validate=None):
        self.cuda = cuda
        tboarddir = os.path.join('/new_data/gpu/ayushya/tblogs/COM-mult/SGD', datetime.datetime.now().strftime('%b%d_%H-%M-%S')+'_'+socket.gethostname())
    ...
    def validate(self):
        ...
        n_paf = 26
        n_seg = 7
        val_loss, pose_loss, paf_loss, seg_loss = 0, 0, 0, 0
        valid_vis_param = 10
        ...
        for batch_idx, (data, targetpose, targetpaf,targetseg, targetpath) in tqdm.tqdm(
            ...
            scorepose1, scorepaf1, scorepose2, scorepaf2, scoreseg1, scoreseg2 = self.model(data)
            ...
            losspaf2 = paf_loss_test(scorepaf2, targetpaf,
                                   size_average=self.size_average)
            lossseg1 = cross_entropy2d(scoreseg1, targetseg,
                                   size_average=self.size_average)
            lossseg2 = cross_entropy2d(scoreseg2, targetseg,
                                   size_average=self.size_average)
            ...
            # losspaf = paf_loss_test(scorepaf, targetpaf,
                                   # size_average=self.size_average)
            wtpose = 14.0*scorepose2.size()[2]*scorepose2.size()[3]/2 
            wtpaf = 26.0*scorepaf2.size()[2]*scorepaf2.size()[3]/2
            ...
            seg_loss += float(lossseg.data[0]) / len(data)
            ...
            if(len(vis_paf)<valid_vis_param):
                ...
                seg_pred = scoreseg2.data.max(1)[1].cpu().numpy()[:, :, :]
                ...
                for img, lt_po, lp_po ,lt_pf ,lp_pf, lt_sg,lp_sg in zip(imgs, pose_lab, pose_pred, paf_lab, paf_pred,\
                    ...
                    viz = fcn.utils.visualize_segmentation(
                        lbl_pred=lp_sg, lbl_true=lt_sg, img=img, n_class=n_seg)
                    vis_seg.append(viz)
            ...
            for posep, pafp, patht, segp, segt in zip(pose_pred,paf_pred,targetpath, seg_pred, seg_true):
                metout = paf_pose_metric(posep, pafp, patht)
                label_trues.append(segt)
                label_preds.append(segp)
        ...
        val_loss /= len(self.val_loader)
        metpose = np.mean(metls, axis=0)
        metls = []
        for lt, lp in zip(label_trues, label_preds):
            acc, acc_cls, mean_iu, fwavacc = \
                torchfcn.utils.label_accuracy_score(
                    [lt], [lp], n_class=n_seg)
            metls.append((acc, acc_cls, mean_iu, fwavacc))
        ...
        with open(osp.join(self.out, 'log.csv'), 'a') as f:
            elapsed_time = \
                datetime.datetime.now(pytz.timezone('Asia/Tokyo')) - \
                self.timestamp_start
            log = [self.epoch, self.iteration] + ['']*8 + \
                  [val_loss] + [pose_loss] + [paf_loss] + [seg_loss] + [metpose] + metls.tolist() + [elapsed_time]
            log = map(str, log)
            f.write(','.join(log) + '\n')
        ...
        self.writer.add_scalars('valid', {"loss": val_loss,
                                         ...
                                         "acc_cls": metls[1],
                                         "mean_iu": metls[2],
                                         "fwavacc": metls[3],}, self.iteration)
    ...
    def train_epoch(self):
        ...
        for batch_idx, (data, targetpose, targetpaf,targetseg, targetpath) in tqdm.tqdm(
            ...
            if (self.iteration % 10)==0:
                metrics = []
                lbl_pred = scoreseg2.data.max(1)[1].cpu().numpy()[:, : ,:]
            ...
            if (self.iteration % 100)==0:
                ...
                # self.writer.add_scalar('trainpytor_avg_met', np.mean(rmet), self.iteration)
                self.writer.add_scalars('train_avg_loss', {"total": np.mean(ravg),
                                                          "pose": np.mean(poseavg),
                                                          "paf": np.mean(pafavg),
                                                          "seg": np.mean(segavg),}, self.iteration)
