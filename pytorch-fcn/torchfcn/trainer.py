...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in tqdm.trange(self.epoch, max_epoch,
            ...
            if self.iteration >= self.max_iter:
                break
class Trainer_wo_crop(object):

    def __init__(self, cuda, model, optimizer,
                 train_loader, val_loader, out, max_iter,
                 size_average=False, interval_validate=None):
        self.cuda = cuda
        tboarddir = os.path.join('runs/wo_crop', datetime.now().strftime('%b%d_%H-%M-%S')+'_'+socket.gethostname())
    ...
    def validate(self):
        ...
        for batch_idx, (data, target) in tqdm.tqdm(
            ...
            data, target = Variable(data, volatile=True), Variable(target)
            score = self.model(data)
            # score = crop_pred(score,target)

            # print(score.size(),target.size(),data.size())
            
            #_____________LOSS____________
            ...
            #_____________Visualization and Metrics_________________
            
            lbl_pred = score.data.max(1)[1].cpu().numpy()[:,:,:]#[:, pad_h1:target.size()[1], pad_w1:target.size()[2]]
            lbl_true = target.data.cpu()
        ...
        #_____________ Metrics_________________
        metrics = torchfcn.utils.label_accuracy_score(
            label_trues, label_preds, n_class)
        ...
        #_____________logging_________________
        ...
        out_file = osp.join(out, 'iter%012d.jpg' % self.iteration)
        scipy.misc.imsave(out_file, fcn.utils.get_tile_image(visualizations))
        self.writer.add_image('Image', fcn.utils.get_tile_image(visualizations), self.iteration)
        val_loss /= len(self.val_loader)
        ...
        self.writer.add_scalars('valid', {"loss": val_loss,
                                         ...
                                         "acc_cls": metrics[1],
                                         "mean_iu": metrics[2],
                                         "fwavacc": metrics[3]}, self.iteration)
        #_____________saving the best model_________________
    ...
    def train_epoch(self):
        ...
        for batch_idx, (data, target) in tqdm.tqdm(
            ...
            data, target = Variable(data), Variable(target)
            self.optim.zero_grad()
            #_____________ forward pass _________________
            ...
            #_____________ loss _________________
            ...
            #_____________ metrics _________________
                metrics = []
                lbl_pred = score.data.max(1)[1].cpu().numpy()[:, : ,:]
                lbl_true = target.data.cpu().numpy()
                for lt, lp in zip(lbl_true, lbl_pred):
            ...
            #_____________ logs _________________
                ...
                self.writer.add_scalars('train', {"loss": loss.data[0],
                                         "acc": metrics[0],
                                         "acc_cls": metrics[1],
                                         "mean_iu": metrics[2],
                                         "fwavacc": metrics[3]}, self.iteration)
...
# def adjust_lr(optimizer, epoch,gamma,lr):
...
class TrainerPAF(object):
    ...
    def __init__(self, cuda, model, optimizer,scheduler,
                 ...
                 size_average=False, interval_validate=None):
        self.cuda = cuda
        tboarddir = os.path.join('/new_data/gpu/ayushya/tblogs/PAF_test', datetime.datetime.now().strftime('%b%d_%H-%M-%S')+'_'+socket.gethostname())
        self.writer = SummaryWriter(tboarddir)
        self.model = model
        self.optim = optimizer
    ...
    def validate(self):
        ...
        for batch_idx, (data, targetpose, targetpaf, targetpath) in tqdm.tqdm(
            ...
            if len(vis_pose) < valid_vis_param:
                ...
                pose_pred = scorepose.data.max(1)[0].cpu().numpy()
                scorepaf_ = torch.abs(scorepaf)
                paf_pred = scorepaf_.data.max(1)[0].cpu().numpy()
                pose_lab = targetpose.data.max(1)[0].cpu().numpy()
                targetpaf_ = torch.abs(targetpaf)
                paf_lab = targetpaf_.data.max(1)[0].cpu().numpy()
...
#                         vis_paf.append(viz)
            ...
            if (self.iteration % 100)==0:
                ...
                for name, param in self.model.named_parameters():
                    ...
                    self.writer.add_histogram(name, param.grad.clone().cpu().data.numpy(), self.iteration)
                # ravg = []
                # rmet = []
                # self.writer.add_scalar('train_metric', metrics, self.iteration)
